...
import {
  FaInstagram, FaTwitter, FaFacebook, FaStar, FaAward, FaHeart, FaRegHeart, FaSpinner, FaArrowLeft,
  FaTrophy, FaGamepad, FaUser, FaLightbulb, FaCar, FaHandHoldingHeart, FaTheaterMasks, FaUsers,
  FaExclamationTriangle, FaGlobe, FaHashtag, FaFilm, FaQuoteLeft, FaNewspaper, FaMicrophone, FaDumbbell,
  FaPalette, FaChartLine, FaYoutube, FaPlay, FaCalendarAlt, FaMapMarkerAlt, FaMoneyBillWave, FaTv,
  FaChevronRight, FaChild, FaLinkedin, FaPinterest, FaRss, FaBriefcase, FaGraduationCap, FaHome,
  FaHeartbeat, FaBook, FaCertificate, FaMask, FaRing, FaGlasses, FaCamera, FaMusic, FaVolumeUp,
...
function getImageUrl(imageObj: any): string {
  ...
  if (typeof imageObj === 'string') {
    ...
    if (imageObj.startsWith('/') && !imageObj.startsWith('//')) return `${TMDB_IMAGE_BASE}${imageObj}`;
    return imageObj;
  }
  if (typeof imageObj === 'object') {
    if (imageObj.url && typeof imageObj.url === 'string' && imageObj.url.startsWith('http')) return imageObj.url;
    if (imageObj.fallback) return imageObj.fallback;
    if (imageObj.tmdbPath && imageObj.tmdbPath.startsWith('/')) return `${TMDB_IMAGE_BASE}${imageObj.tmdbPath}`;
  }
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 py-3 md:py-4 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 md:gap-4 flex-wrap justify-end min-w-0">
          ...
          {session?.user ? (
            <Link href="/profile" className="group flex items-center gap-2 px-2 md:px-3 py-1.5 md:py-2 rounded-lg hover:bg-white/5 transition flex-shrink-0">
            ...
            <Link href="/signin" className="px-3 md:px-4 py-1.5 md:py-2 text-xs md:text-sm bg-white text-black font-bold rounded-lg hover:bg-white/90 transition flex-shrink-0">
              Sign In
            </Link>
          )}
        </div>
...
function TabNavigation({ activeTab, setActiveTab, hero }: any) {
  ...
  { id: 'trivia', label: 'Trivia', icon: FaLightbulb, count: hero.trivia?.length || 0 },
  { id: 'quotes', label: 'Quotes', icon: FaQuoteLeft, count: hero.quotes?.length || 0 },
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {hero.socialMedia?.instagram?.url && (
                  <motion.a
                    ...
                    whileHover={{ scale: 1.05 }}
                    className="px-6 py-3 bg-white/10 hover:bg-white/20 text-white rounded-lg font-bold transition flex items-center gap-2 backdrop-blur-sm border border-white/20"
                  >
          ...
          <div className="rounded-lg p-5 md:p-6 bg-white/5 border border-white/10 space-y-4 md:space-y-6">
            ...
            {(hero.voiceProfile.languagesFluent || hero.voiceProfile.languages) && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(hero.voiceProfile.languagesFluent || hero.voiceProfile.languages).map((lang: any, i: number) => {
                    ...
                    return (
                      <span 
                        key={i} 
                        className="px-3 py-1 bg-white/10 text-white text-xs rounded-full font-bold"
                        title={typeof lang === 'object' && lang.notes ? lang.notes : undefined}
                      >
    ...
    >
      ...
      {info && Object.keys(info).length > 0 && (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            ...
            {info.currentResidence && <InfoCard label="Current Residence" value={info.currentResidence} icon={FaHome} />}
            {info.nationality && <InfoCard label="Nationality" value={info.nationality} icon={FaGlobe} />}
            {info.ethnicity && <InfoCard label="Ethnicity" value={info.ethnicity} icon={FaGlobe} />}
            
            {/* Personal Traits */}
            {info.personalTraits?.zodiacSign && <InfoCard label="Zodiac Sign" value={info.personalTraits.zodiacSign} icon={FaStar} />}
            {info.personalTraits?.bloodGroup && <InfoCard label="Blood Group" value={info.personalTraits.bloodGroup} icon={FaHeartbeat} />}
            {info.personalTraits?.personalityType && <InfoCard label="Personality Type" value={info.personalTraits.personalityType} icon={FaUser} />}
            
            {/* Career Start */}
            {info.careerStart?.debutYear && <InfoCard label="Debut Year" value={info.careerStart.debutYear.toString()} icon={FaFilm} />}
            {info.careerStart?.debutFilm && <InfoCard label="Debut Movie" value={info.careerStart.debutFilm} icon={FaTv} />}
            {info.careerStart?.yearsActive && <InfoCard label="Years Active" value={`${info.careerStart.yearsActive} Years`} icon={FaCalendarAlt} />}
          </div>
        </motion.section>
      )}

      {/* Education */}
      {info.education && typeof info.education === 'object' && Object.keys(info.education).filter(k => info.education[k]).length > 0 && (
        <motion.section
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            {info.education.schooling && (
              <InfoCard label="Schooling" value={info.education.schooling} icon={FaGraduationCap} />
            )}
            {info.education.highSchool && (
              <InfoCard label="High School" value={info.education.highSchool} icon={FaGraduationCap} />
            )}
            {info.education.college && (
              <InfoCard label="College" value={info.education.college} icon={FaGraduationCap} />
            )}
            {info.education.specialization && (
              <InfoCard label="Specialization" value={info.education.specialization} icon={FaCertificate} />
            )}
          </div>
        </motion.section>
      )}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            {info.familyInfo.maritalStatus && (
              <InfoCard label="Marital Status" value={info.familyInfo.maritalStatus} icon={FaRing} />
            )}
            ...
            {info.familyInfo.family?.mother && (
              <InfoCard label="Mother" value={info.familyInfo.family.mother} icon={FaUser} />
            )}
            
            {/* Siblings */}
            {info.familyInfo.family?.siblings && Array.isArray(info.familyInfo.family.siblings) && info.familyInfo.family.siblings.length > 0 && (
              <InfoCard 
                label="Siblings" 
                value={info.familyInfo.family.siblings.map((s: any) => typeof s === 'string' ? s : s.name).join(', ')} 
                icon={FaUsers} 
              />
            )}
            
            {/* Children */}
            {info.familyInfo.children && Array.isArray(info.familyInfo.children) && info.familyInfo.children.length > 0 && (
              <InfoCard 
                label="Children" 
                value={info.familyInfo.children.map((c: any) => typeof c === 'string' ? c : c.name).join(', ')} 
                icon={FaChild} 
              />
            )}
          </div>
      ...
      {Object.keys(physical).length > 0 && (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            {/* Body Stats */}
            {physical.body?.height && <InfoCard label="Height" value={physical.body.height} icon={FaUser} />}
            {physical.body?.weight?.current && <InfoCard label="Weight" value={physical.body.weight.current} icon={FaDumbbell} />}
            {physical.body?.bodyType && <InfoCard label="Body Type" value={physical.body.bodyType} icon={FaUser} />}
            {physical.body?.bodyMassIndex && <InfoCard label="BMI" value={physical.body.bodyMassIndex} icon={FaHeartbeat} />}
            {physical.body?.bodyFatPercentage && <InfoCard label="Body Fat" value={physical.body.bodyFatPercentage} icon={FaDumbbell} />}
            
            {/* Measurements */}
            {physical.measurements?.chest && <InfoCard label="Chest" value={physical.measurements.chest} icon={FaDumbbell} />}
            {physical.measurements?.waist && <InfoCard label="Waist" value={physical.measurements.waist} icon={FaDumbbell} />}
            {physical.measurements?.biceps && <InfoCard label="Biceps" value={physical.measurements.biceps} icon={FaDumbbell} />}
            {physical.measurements?.thighs && <InfoCard label="Thighs" value={physical.measurements.thighs} icon={FaDumbbell} />}
            
            {/* Appearance */}
            {physical.appearance?.hairColor && <InfoCard label="Hair Color" value={physical.appearance.hairColor} icon={FaUser} />}
            {physical.appearance?.eyeColor && <InfoCard label="Eye Color" value={physical.appearance.eyeColor} icon={FaGlasses} />}
            {physical.appearance?.skinTone && <InfoCard label="Skin Tone" value={physical.appearance.skinTone} icon={FaUser} />}
            
            {/* Footwear */}
            {physical.footwear?.shoeSizeUK && <InfoCard label="Shoe Size (UK)" value={physical.footwear.shoeSizeUK} icon={FaUser} />}
            {physical.footwear?.shoeSizeUS && <InfoCard label="Shoe Size (US)" value={physical.footwear.shoeSizeUS} icon={FaUser} />}
            
            {/* Fitness */}
            {physical.fitnessProfile?.fitnessLevel && <InfoCard label="Fitness Level" value={physical.fitnessProfile.fitnessLevel} icon={FaDumbbell} />}
            {physical.fitnessProfile?.mainTrainer && <InfoCard label="Main Trainer" value={physical.fitnessProfile.mainTrainer} icon={FaUser} />}
            {physical.fitnessProfile?.dailyTrainingHours && <InfoCard label="Training Hours" value={physical.fitnessProfile.dailyTrainingHours} icon={FaDumbbell} />}
            
            {/* Health */}
            {physical.healthMetrics?.bloodGroup && <InfoCard label="Blood Group" value={physical.healthMetrics.bloodGroup} icon={FaHeartbeat} />}
            {physical.healthMetrics?.restingHeartRate && <InfoCard label="Resting Heart Rate" value={physical.healthMetrics.restingHeartRate} icon={FaHeartbeat} />}
          </div>
        </motion.section>
      )}
      ...
      {Object.keys(socialMedia).length > 0 && (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-white font-bold text-sm truncate">
                    {typeof socialMedia.instagram === 'object' && socialMedia.instagram.handle 
                      ? socialMedia.instagram.handle 
                      : 'Follow'}
                  </p>
                  {typeof socialMedia.instagram === 'object' && socialMedia.instagram.followers && (
                    <p className="text-white/40 text-xs">
            ...
            {socialMedia.twitter && (
              <a
                href={typeof socialMedia.twitter === 'string' ? socialMedia.twitter : socialMedia.twitter.url}
                target="_blank"
                rel="noopener noreferrer"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-white font-bold text-sm truncate">
                    {typeof socialMedia.twitter === 'object' && socialMedia.twitter.handle 
            ...
            {socialMedia.youtube && (
              <a
                href={typeof socialMedia.youtube === 'string' ? socialMedia.youtube : socialMedia.youtube.url}
                target="_blank"
                rel="noopener noreferrer"
            ...
            {lifestyle.carCollection.map((car: any, idx: number) => (
              ...
              >
                ...
                <div className="space-y-2 text-white/70 text-xs md:text-sm">
                  ...
                  {car.notes && <p className="text-white/50 text-xs italic mt-2">{car.notes}</p>}
                </div>
              </motion.div>
            ))}
          </div>
            ...
            {lifestyle.properties.map((property: any, idx: number) => (
              ...
              >
                ...
                <div className="space-y-2 text-white/70 text-xs md:text-sm">
                  ...
                  {property.area && <p>📐 <span className="text-white font-medium">{property.area}</span></p>}
                  {property.estimatedValue && <p>💰 <span className="text-white font-medium">{property.estimatedValue}</span></p>}
                  {property.acquired && <p>📅 <span className="text-white font-medium">Acquired {property.acquired}</span></p>}
                  {property.description && <p className="text-white/50 text-xs mt-2">{property.description}</p>}
                </div>
                {property.primaryResidence && (
                  <div className="mt-3 px-2 py-1 bg-green-500/20 border border-green-500/30 text-green-400 rounded text-xs font-bold inline-block">
                    Primary Residence
                  </div>
                )}
              </motion.div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                <span
                  className={`px-2 py-0.5 rounded text-xs font-bold whitespace-nowrap ${
                    award.won ? 'bg-green-500/20 text-green-300' : 'bg-blue-500/20 text-blue-300'
                  }`}
                >
              ...
              <p className="text-white/60 text-xs md:text-sm line-clamp-1">
                {award.category} {award.film && `• ${award.film}`} {award.year && `(${award.year})`}
              </p>
              {award.givenBy && (
          ...
          >
            ...
            >
              ...
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
                onClick={() => setSelectedImage((p) => (p === 0 ? images.length - 1 : (p ?? 0) - 1))}
                className="absolute left-4 top-1/2 -translate-y-1/2 p-2 md:p-3 rounded-full bg-white/20 hover:bg-white/40 transition text-white"
              >
              ...
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
                onClick={() => setSelectedImage((p) => (p === images.length - 1 ? 0 : (p ?? 0) + 1))}
                className="absolute right-4 top-1/2 -translate-y-1/2 p-2 md:p-3 rounded-full bg-white/20 hover:bg-white/40 transition text-white"
              >
                <FaChevronRight />
              </motion.button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        {trivia.map((item: any, idx: number) => (
          ...
          >
            ...
            {item.category && (
              <span className="mt-2 inline-block px-2 py-1 bg-white/10 text-white text-xs rounded-full font-medium">
                {item.category}
              </span>
            )}
          </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        {quotes.map((quote: any, idx: number) => (
          <motion.div
            ...
            className="rounded-lg p-4 md:p-6 bg-white/5 border border-white/10 hover:border-white/20 transition"
          >
            <p className="text-white/80 text-sm md:text-base italic leading-relaxed">"{quote.text}"</p>
            {quote.source && (
...
function ContributeTab({ hero }: any) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    // Example: await fetch('/api/contribute', { method: 'POST', body: JSON.stringify({ heroId: hero.id, message, image, trivia, quote }) });
    setSubmitted(true);
  };
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        <p className="text-white/80 text-base md:text-lg">Thank you for your contribution!</p>
      </motion.div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <textarea
            ...
            className="w-full p-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-white/40 focus:outline-none focus:border-white/30"
            rows={3}
        ...
        <div>
          <label className="block text-white/70 text-sm font-medium mb-2">Trivia</label>
          <textarea
            value={trivia}
        ...
        <div>
          <label className="block text-white/70 text-sm font-medium mb-2">Quote</label>
          <textarea
            value={quote}
